                    ...
                    <ul>
                        <c:forEach items="${menu}" var="m" varStatus="ms">
                            <c:if test="${m.upmenuid==-1}">
                                <li><input type="checkbox" name="menu" value="${m.menuid}" onclick="checkChild('${m.menuid}')" id="head${m.menuid}" />${m.menuname}
                                    <ul>
                                    <c:forEach items="${menu}" var="mChild" >
                                        <c:if test="${mChild.upmenuid==m.menuid}">
                                            <li>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" onclick="checkHead('${m.menuid}')" class="child${m.menuid}" name="menu" value="${mChild.menuid}"  />${mChild.menuname}</li>
                                        </c:if>
                                    </c:forEach>
                                    </ul>
                                </li>
